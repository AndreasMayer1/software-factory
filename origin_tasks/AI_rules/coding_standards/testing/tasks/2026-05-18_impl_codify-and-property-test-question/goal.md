...
covers:
  ...
  sections: []
scope_description: "Backfill property test for codify and property test question (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Codify and property test question

## Objective

Codify the per-`QuestionType` invariant matrix for `Question` *before* writing the property test. The current `create` validation contains dead-code branches (`if (choiceOptions == null && timeOptions == null && likertOptions == null && timeInputType == null)` after positive checks have returned). Rewrite as an explicit invariant table mapping each of the 7 `QuestionType` variants to its required nested-options field (or to a typed `throwsA` exception). Then add a `glados` property test exercising: (b) `QuestionType`-switch totality on `create` and `fromJson` — every variant either accepts a matching nested-options instance or throws a typed exception; (c) non-empty field constraints; (e) JSON round-trip per variant.
...
- Target type / source: `lib/core/domain/entities/questionnaire_plan_entities/question.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.combine(QuestionType, matching-options pair)
...
- any.combine(QuestionType, matching-options pair)
