...
covers:
  ...
  sections: []
scope_description: "Backfill property test for codify and property test questionnaire plan (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Codify and property test questionnaire plan

## Objective

Codify and align two inconsistent invariants for `QuestionnairePlan` *before* writing the property test: (1) `questionnaireUuids.isEmpty` is rejected in `fromJson` but the guard is commented out in `create` — align the two construction paths; (2) `endDate >= startDate` ordering invariant — enforce in factory. Then add a `glados` property test covering (c) non-empty name, the codified date-ordering invariant, (e) JSON round-trip, and (f) `questionnaireUuids` list preservation.
...
- Target type / source: `lib/core/domain/entities/questionnaire_plan_entities/questionnaire_plan.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.list(any.nonEmptyString), date generators
...
- any.list(any.nonEmptyString), date generators
