...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test pairing identity (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test pairing identity

## Objective

Add a `glados` property test for `PairingIdentity` exercising (c) the non-empty UUID invariant and the Equatable identity property (two `PairingIdentity` values with the same UUID compare equal).
...
- Target type / source: `lib/core/domain/entities/pairing_identity.dart`
