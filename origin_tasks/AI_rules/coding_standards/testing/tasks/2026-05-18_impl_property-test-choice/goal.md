...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test choice (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test choice

## Objective

Add a `glados` property test for `Choice` exercising (c) the non-empty `text` and `questionUuid` invariants and (e) the JSON round-trip (note: `toJson` includes `dataVersion`; verify the round-trip preserves all generator-controlled fields).
...
- Target type / source: `lib/core/domain/entities/questionnaire_plan_entities/choice.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.nonEmptyString
