...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test transfer chunk (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test transfer chunk

## Objective

Add a `glados` property test for `TransferChunk` exercising (a) uint16 header range invariants `[0, 65535]`, (c) payload length, and the two round-trip pairs: `fromBytes(toBytes(x)) == x` and `fromBase64QrString(toBase64QrString(x)) == x`. This is the highest-value property in the inventory because it crosses a wire-format boundary (base64url over QR).
...
- Target type / source: `lib/features/therapist/data_transfer/domain/value_objects/transfer_chunk.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.intInRange(0, 65535), any.list(any.intInRange(0, 255)).map(Uint8List.fromList)
...
- any.intInRange(0, 65535), any.list(any.intInRange(0, 255)).map(Uint8List.fromList)
