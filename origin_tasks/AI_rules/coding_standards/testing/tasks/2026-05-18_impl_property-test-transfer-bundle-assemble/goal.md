...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test transfer bundle assemble (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test transfer bundle assemble

## Objective

Add a `glados` property test for `TransferBundle.assemble` covering (f) four algebraic invariants: (1) `assemble(... isPrivate=true ...).entryRefs` is always empty for private entries regardless of `ScopeVariant` (idempotence-of-filter); (2) `defaultScope.entryRefs ⊆ widened.entryRefs ⊆ all-candidate-uuids` (monotonicity across scope-variant widening); (3) `entryRefs` is duplicate-free given duplicate-free input; (4) any non-empty `voiceRecordingRefs` with `TransferChannel.qr` always throws (QR-channel rejection invariant).
...
- Target type / source: `lib/core/domain/entities/tracking_entry_entities/transfer_bundle.dart`
- Property-test categories covered: see Objective above.
- Generators required: custom: entry-set + ScopeVariant + isPrivate flag combination generator
...
- custom: entry-set + ScopeVariant + isPrivate flag combination generator
