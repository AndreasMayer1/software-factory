...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test enum totality batch (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test enum totality batch

## Objective

Add a `glados` parameterised property test that covers (b) enum totality + (e) encode/decode round-trip + unknown-variant rejection for all seven domain enums: `QuestionType`, `TimeInputType`, `TimeIntervalType`, `TimeLabelType`, `TransferChannel`, `ScopeVariant`, `TransferDetectionZone`. Single test file with one property per enum — keeps cognitive load low and the file structure self-documenting.

Cluster 2 — enum totality batch (per inventory recommendation).
...
- Target type / source: `lib/core/domain/entities/questionnaire_plan_entities/ (multiple) + lib/features/.../transfer_*.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.oneOf over each enum's values
...
- any.oneOf over each enum's values
