...
covers:
  ...
  sections: []
scope_description: "Backfill property test for property test scanner hardware tier (REQ-PROC-002 AC-03)."
release_description: ""
...
---
# Goal: Property test scanner hardware tier

## Objective

Add a `glados` property test for `ScannerHardwareTier` exercising (b) enum totality — every variant has a defined `pairingTierCode` mapping and the mapping only emits values in the documented invariant set (`{2, 3}`).
...
- Target type / source: `lib/features/therapist/data_transfer/domain/value_objects/scanner_hardware_tier.dart`
- Property-test categories covered: see Objective above.
- Generators required: any.oneOf(ScannerHardwareTier.values)
...
- any.oneOf(ScannerHardwareTier.values)
